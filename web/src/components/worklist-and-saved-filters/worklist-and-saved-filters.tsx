...
import { Button, Field, Link, Panel, StatusBadge } from '@/components/ui'
...
function FilterSelect({ label, value, options, onChange }: { label: string; value: string; options: readonly { value: string; label: string }[]; onChange: (value: string) => void }) {
  return <label className="form-control min-w-48 flex-1 gap-2"><span className="label-text font-medium">{label}</span><select className="select select-bordered min-h-11" value={value} onChange={(event) => onChange(event.target.value)}><option value="">All</option>{options.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>
}
