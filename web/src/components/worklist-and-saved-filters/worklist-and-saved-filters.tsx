...
export function WorklistAndSavedFilters({
  ...
  const tableCaptionId = `${useId()}-worklist-caption`
    ...
    <Panel title="Worklist and saved filters" description="Fixture-backed worklist behavior with explicit scope, ownership, state, and eligibility context.">
      <div className="mt-4 space-y-5">
        ...
        {status !== 'loading' && status !== 'unavailable' && status !== 'rejected' && filteredItems.length > 0 ? <>
          <div role="region" aria-labelledby={tableCaptionId} tabIndex={0} data-a11y-scroll-region className="overflow-x-auto"><table className="table min-w-full"><caption id={tableCaptionId} className="caption-top pb-3 text-left text-sm font-semibold">Operational work items</caption><thead className="bg-base-200/70">{table.getHeaderGroups().map((headerGroup) => <tr key={headerGroup.id}><th className="text-base-content" scope="col"><span className="sr-only">Select</span></th>{headerGroup.headers.map((header) => <th className="text-base-content" key={header.id} scope="col">{header.isPlaceholder ? null : <button type="button" className="font-semibold underline-offset-2 hover:underline focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary" onClick={header.column.getToggleSortingHandler()}>{flexRender(header.column.columnDef.header, header.getContext())}{header.column.getIsSorted() ? ` (${header.column.getIsSorted() === 'asc' ? 'ascending' : 'descending'})` : ''}</button>}</th>)}</tr>)}</thead><tbody>{table.getRowModel().rows.map((row) => <tr key={row.id} data-selected={row.getIsSelected() ? 'true' : undefined} className={`border-b border-base-300/80 odd:bg-base-200/35 transition-colors hover:bg-info/10 ${row.getIsSelected() ? 'bg-primary/10' : ''}`}><td><input className="checkbox checkbox-sm focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary" type="checkbox" aria-label={`Select ${row.original.record.label}`} checked={row.getIsSelected()} disabled={!row.original.bulkEligible} onChange={row.getToggleSelectedHandler()} /></td>{row.getVisibleCells().map((cell) => cell.column.id === 'record' ? <th key={cell.id} scope="row">{flexRender(cell.column.columnDef.cell, cell.getContext())}</th> : <td key={cell.id}>{flexRender(cell.column.columnDef.cell, cell.getContext())}</td>)}</tr>)}</tbody></table></div>
...
function FilterSelect({ label, value, options, onChange }: { label: string; value: string; options: readonly { value: string; label: string }[]; onChange: (value: string) => void }) {
  return <div className="form-control min-w-0 w-full flex-1 gap-2"><span className="font-medium text-base-content">{label}</span><Select aria-label={label} value={value} onChange={(event) => onChange(event.target.value)}><option value="">All</option>{options.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</Select></div>
}
