...
export function VersionConflictDialog({ conflict, onClose, onRetry, onRefresh, onNewIdentity }: { conflict: VersionConflictSnapshot; onClose: () => void; onRetry?: () => void; onRefresh?: () => void; onNewIdentity?: () => void }) {
  ...
  const dialogRef = useCallback((dialog: HTMLDivElement | null) => {
    if (!dialog) return
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-neutral/50 p-4" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) close() }}>
    <div ref={dialogRef} onKeyDown={handleKeyDown} role="dialog" aria-modal="true" aria-labelledby={titleId} aria-describedby={descriptionId} className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-box bg-base-100 p-6 shadow-xl">
      <div className="flex items-start justify-between gap-4"><div><p className="text-sm font-semibold uppercase tracking-wide text-base-content/75">CON-SCR-01</p><h2 id={titleId} className="mt-1 text-xl font-semibold">The record changed before this action</h2></div><Button variant="ghost" size="sm" onClick={close}>Close</Button></div>
