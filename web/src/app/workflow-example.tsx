...
export function WorkflowExample() {
  ...
  const { register, handleSubmit, setError, setFocus, formState: { errors } } = useForm<MaterialActionFormValues>({ resolver: zodResolver(schema), defaultValues: { businessIdentity: 'SET-1042', description: 'Apply approved supplier settlement adjustment', amount: '1,250.00', currency: 'THB', lineReference: 'SET-1042-L1' } })
  const formIssues = Object.entries(errors).map(([field, error]) => ({ id: `field-${field}`, category: 'field' as const, code: 'INVALID_FIELD', message: error?.message ?? 'Review this field.', targetId: field, targetLabel: field === 'lineReference' ? 'Line reference' : field[0].toUpperCase() + field.slice(1) }))
  ...
  const restoreTrigger = () => document.getElementById('workflow-conflict-trigger')?.focus()
  ...
  return <div className="space-y-6">
    ...
    {conflictOpen ? <VersionConflictDialog conflict={conflictFixture} onClose={() => { setConflictOpen(false); restoreTrigger() }} onRefresh={() => setOutcome('Authoritative state refreshed: current version 8 is ready for review.')} onRetry={() => { setConflictOpen(false); setOutcome('Retry blocked until the refreshed version is reviewed.'); restoreTrigger() }} onNewIdentity={() => { setConflictOpen(false); setOutcome('A new business identity must be created for an identity-content conflict.'); restoreTrigger() }} /> : null}
  </div>
}
