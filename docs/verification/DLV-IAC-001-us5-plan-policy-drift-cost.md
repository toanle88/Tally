...
| Security scan | `scripts/verify/terraform-security.sh` | Checkov scans each root; only observed exact environment/rule/path exceptions from the rationale, owner, and expiry manifest are filtered. Production-reference cannot be excepted. |
